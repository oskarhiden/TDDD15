...
In general the smoothed is also better than most prob path, Constrain, valid path, prob i sell or ecual accurate
...
```
In the first iterations the entrpy drops since we get more information, and therfore we get a better prediction. But after a couple of iterations the entropy increases again. Therfore we do not always get a better prediction with more observations. For some t the entropy is 0, and then we are sure of our predicton. But then the entropy increases agin. depending on the combination of observations we could get a better prediction. 
